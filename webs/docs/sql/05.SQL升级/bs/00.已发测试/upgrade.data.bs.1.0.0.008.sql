...
BEGIN
  ...
  num:=0;
	select count(1) into num from bs_qxmx where ACTION='/login' and METHOD='tdIndex' and STYP='1' and SID='M00000';     
	if num = 0 then
		insert into bs_qxmx (pkid,mkid,action,method,accesstag,remark,styp,sid)
		values(bs_qxmx_0.nextval,'0','/login','tdIndex',0,'td前台登录页面',1,'M00000');
	end if;
	
	num:=0;
	select count(1) into num from bs_qxmx where ACTION='/login' and METHOD='doTdLogin' and STYP='1' and SID='M00000';     
	if num = 0 then
		insert into bs_qxmx (pkid,mkid,action,method,accesstag,remark,styp,sid)
		values(bs_qxmx_0.nextval,'0','/login','doTdLogin',0,'td前台登录',1,'M00000');
	end if;
	
	num:=0;
	select count(1) into num from bs_qxmx where ACTION='/' and METHOD='jjbuyggInfo' and STYP='1' and SID='M00004';     
	if num = 0 then
		insert into BS_QXMX ( MKID, ACTION, METHOD, ACCESSTAG, REMARK, STYP, SID)
  values ( '0', '/', 'jjbuyggInfo', 0, '竞价采购公告详情', 1, 'M00004');
	end if;
	
	num:=0;
	select count(1) into num from bs_qxmx where ACTION='/' and METHOD='jjsellggInfo' and STYP='1' and SID='M00004';     
	if num = 0 then
		insert into BS_QXMX ( MKID, ACTION, METHOD, ACCESSTAG, REMARK, STYP, SID)
	values ( '0', '/', 'jjsellggInfo', 0, '竞价销售公告详情', 1, 'M00004');
	end if;
	
	num:=0;
	select count(1) into num from bs_qxmx where ACTION='/exp/querybusiness/common' and METHOD='xjggInfo' and STYP='1' and SID='M00004';     
	if num = 0 then
		insert into BS_QXMX ( MKID, ACTION, METHOD, ACCESSTAG, REMARK, STYP, SID)
	values ( 'M017005', '/exp/querybusiness/common', 'xjggInfo', 0, '询价公告详情', 1, 'M00004');
	end if;
	
	
  select count(1) into num from bs_qxmx where ACTION='/tdp/materials/index' and METHOD='filter' and STYP='1' and SID='M00004';     
  if num = 0 then
	insert into bs_qxmx(pkid,mkid,action,method,accesstag,remark,styp,sid)
	values(bs_qxmx_0.nextval,'0','/tdp/materials/index','filter','0','物资调剂-首页-过滤条件','1','M00004');
  end if;	
  
  update bs_xtmk set qxrole = '005-1' where sid='M00004' and mid='M0245';

  
  --合同跟踪-修改支付计划 by zhuchunhong 2013-01-02
  select count(1) into num from bs_qxsz where mkid='M015520' and sid='M00004'; 
  if num = 0 then
	insert into bs_qxsz (MKID, MID, MNAME, NAME, URL, INURL, ISUSE, ORDERNO, STYP, PARENT, SID)
	values ('M015520', 'M0155', '合同支付', '支付计划', '/tdp/trail/payplan/list.do', '/tdp/trail/payplan/list.do#/tdp/trail/payplan/getPayplan.do', 1, 50, 1, '', 'M00004');
  end if;
  select count(1) into num from bs_qxsz where mkid='M01552010' and sid='M00004'; 
  if num = 0 then
	insert into bs_qxsz (MKID, MID, MNAME, NAME, URL, INURL, ISUSE, ORDERNO, STYP, PARENT, SID)
	values ('M01552010', 'M0155', '合同支付', '编辑', '', '', 2, 50, 1, 'M015520', 'M00004');
  end if;
  select count(1) into num from bs_qxmx where mkid='M015520' and sid='M00004' and action='/tdp/trail/payplan' and method='list';  
  if num = 0 then
	insert into bs_qxmx (PKID, MKID, ACTION, METHOD, ACCESSTAG, REMARK, STYP, SID)
	values (bs_qxmx_0.nextval, 'M015520', '/tdp/trail/payplan', 'list', 2, '支付计划合同列表', 1, 'M00004');
  end if;
  select count(1) into num from bs_qxmx where mkid='M01552010' and sid='M00004' and action='/tdp/trail/payplan' and method='getPayplan';  
  if num = 0 then
	insert into bs_qxmx (PKID, MKID, ACTION, METHOD, ACCESSTAG, REMARK, STYP, SID)
	values (bs_qxmx_0.nextval, 'M01552010', '/tdp/trail/payplan', 'getPayplan', 2, '合同信息和支付计划', 1, 'M00004');
  end if;
  select count(1) into num from bs_qxmx where mkid='M01552010' and sid='M00004' and action='/tdp/trail/payplan' and method='doEdit';
  if num = 0 then
	insert into bs_qxmx (PKID, MKID, ACTION, METHOD, ACCESSTAG, REMARK, STYP, SID)
	values (bs_qxmx_0.nextval, 'M01552010', '/tdp/trail/payplan', 'doEdit', 2, '修改支付计划', 1, 'M00004');
  end if;
    -------------------------------------------------------------
    -- 升级文件到此结束
     -- 以下不允许出现升级SQL
