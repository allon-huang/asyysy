...
begin
	...
	set @num=0;
	set @tableName='BD_PACKAGE_EXT';
	set @columnName='FHXJC';
	select count(0) into @num from information_schema.COLUMNS where TABLE_NAME=@tableName and COLUMN_NAME=@columnName and TABLE_SCHEMA=DATABASE();
	if @num=0 then
		set @dsql=concat('alter table ',@tableName,' add ',@columnName,' varchar(500) COMMENT ON COLUMN ',@tableName,'.',@columnName ,'IS',@remarks);
		PREPARE drun from @dsql; -- 动态执行
		EXECUTE drun;
		DEALLOCATE PREPARE drun;
	end if;
	...
	select count(0) into @num from information_schema.COLUMNS where TABLE_NAME=@tableName and COLUMN_NAME=@columnName and TABLE_SCHEMA=DATABASE();
	if @num=0 then
		set @dsql=concat('alter table ',@tableName,' add ',@columnName,' varchar(100) COMMENT ON COLUMN ',@tableName,'.',@columnName ,'IS',@remarks);
		PREPARE drun from @dsql; -- 动态执行
		EXECUTE drun;
		DEALLOCATE PREPARE drun;
	end if;
